...
function pushEvent(events, e, obj, calendarName, startDate, endDate) {
  events.push({
    start: startDate,
    end: endDate,
    summary: e.summary ?? '',
...
function expandRecurring(e, obj, calendarName, rangeStart, rangeEnd, events) {
  try {
    ...
    while ((next = iter.next()) && count++ < maxIter) {
      ...
      pushEvent(events, e, obj, calendarName, new Date(startMs), new Date(endMs));
    }
...
// קורא אירועים מכל לוחות השנה (לבדיקת זמינות)
export async function fetchAllEvents(start, end) {
  ...
  for (const cal of calendars) {
    try {
      ...
      all.push(...parseObjects(objects, cal.displayName, start, end));
    } catch { /* מדלגים על לוח שלא נטען */ }
  }
  return all;
}
...
export async function createCalendarEvent({ studentName, phone, subject, startDate, endDate }) {
  const { client, calendars } = await connect();
  const calendar = findTargetCalendar(calendars);
  const uid = uuidv4();
